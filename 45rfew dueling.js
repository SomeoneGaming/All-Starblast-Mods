      ...
      if (!ship.custom.init){ 
        ...
        game.ships[i].setUIComponent({ 
          ...
          components: [
            ...
            { type: "text",position:[0,30,100,60],value:"Options [W]",color:"#cde"},
          ]
        });   
        ...
        game.ships[i].setUIComponent({ 
          ...
          shortcut: "J",
          components: [
            { type:"box",position:[0,0,190,100],fill:"rgba(68, 85, 102, 0)",stroke:"#cde",width:5},
            { type: "text",position:[0,30,100,60],value:"Restore [J]",color:"#cde"},
          ]
        });       
        ship.custom.tree = 0;
...
this.event = function(event, game){
  var ship = event.ship, ship_level = Math.trunc(ship.type/100), max_crystals = 980,  
  new_type, tree = ship.custom.tree, index = -1;
    ...
    case "ui_component_clicked":
      ...
      if (component == "Options"){
        if (ship.custom.option_screen === true){
          ...
          ship.setUIComponent({id: "Previous ship",visible: false});        
          ship.setUIComponent({id: "Reset",visible: false});  
          ship.setUIComponent({id: "Spectate",visible: false});  
          ship.setUIComponent({id: "A-Speedster",visible: false});  
          ship.custom.option_screen = true;
        }
      }  
        ...
        if (index >= 0) {
          ...
          if (ship_level < 7) ship.set({shield:999, stats:88888888});      
          ship.set({type:new_type, shield:999});
        } if (ship.type == 101) ship.set({type:704});
      }
      else if (component == "Spectate"){
        if (ship.type != 102)
          ship.set({type:102, crystals:20});
        else ship.set({type:101});
      }
      else if (component == "Reset"){
        if (ship.type != 101)
        ship.set({type:101, crystals:20, shield:100});
      }
      else if (component == "A-Speedster"){
        if (ship.type != 605)
        ship.set({type:605, crystals:720, shield:990});
      }
      else if (component == "Restore"){
        ship.set({shield:999, crystals:((ship_level||0)**2)*20,generator:999,stats:ship.stats});}
      else if (component == "Admin ship"){
       ship.set({type:191,crystals:20,generator:10000});}
      break;
    case "ship_spawned": 
